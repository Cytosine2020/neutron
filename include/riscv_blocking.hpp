...
private:
    ...
    Graph<UXLenT> blocking(UXLenT guest, void *host, usize size) {
        ...
        while (offset < size) {
            ...
            usize inc = visit_in_memory(reinterpret_cast<riscv_isa::Instruction *>(inst),
                                        size - offset);
            if (inc == 0) break;
            offset += inc;
        }
...
#define _neutron_return_inst_len(NAME, name) \
        RetT visit_##name##_inst(const riscv_isa::NAME##Inst *inst) { \
            return return_inst_len(inst); \
        }
...
#undef _neutron_return_inst_len
    ...
    static Graph<UXLenT>
    build(UXLenT guest, void *host, usize size, BranchMapT &branch, BranchMapT &indirect) {
