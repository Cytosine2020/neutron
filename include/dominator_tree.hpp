...
private:
    ...
    DominatorTree(GraghT &graph, VertexT entry) :
            entry{entry}, graph{graph}, vertex{}, vertex_info{} {};
    ...
    std::map<VertexT, VertexT> semi_nca() {
        ...
        for (auto block = graph.begin(); block != graph.end(); ++block) {
            ...
            if (v != entry) {
                ...
                dominator.emplace(v, v_info == vertex_info.end() ? entry
                                                                 : vertex[v_info->second.idom]);
            }
        }
